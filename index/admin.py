...
class LabelAdmin(admin.ModelAdmin):
    ...
    # 设置可搜索的字段并在Admin后台数据生成搜索框，如有外键应使用双下划线连接两个模型字段
...
class SongAdmin(admin.ModelAdmin):
    ...
    # 设置过滤器, 在后台数据的右侧生成导航栏， 如有外键使用双下划线连接两个模型的字段
...
class DynamicAdmin(admin.ModelAdmin):
    list_display = ['dynamic_id', 'song', 'dynamic_play', 'dynamic_search', 'dynamic_down']
    search_fields = ['song']
    list_filter = ['dynamic_play', 'dynamic_search', 'dynamic_down']
    ordering = ['dynamic_id']
...
class CommentAdmin(admin.ModelAdmin):
    list_display = ['comment_id', 'song', 'comment_user', 'comment_date', 'comment_text']
